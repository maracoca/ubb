...
class RentalService():
    ...
    def generate_rentals(self):
        if self.rental_repository.get_all_objects() == []:
    ...
    #         book_rentals[rental.book] += 1
    #     sorted_books = sorted(book_rentals, reverse=True)
    #     for book in sorted_books:
    #         print(book)
    #     return sorted_books

    def filter_rentals(self, client_id, book_id):
        result = []
        for rental in self.rental_repository.get_all_objects():
            if client_id is not None and rental.client_id != client_id:
                continue
            if book_id is not None and rental.book_id != book_id:
                continue
            result.append(rental)
        return result

    def delete_rental(self, rental_id):
        self.rental_repository.remove(rental_id)